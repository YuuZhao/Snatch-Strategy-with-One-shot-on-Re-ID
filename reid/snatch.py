...
class EUG():
    def __init__(self, model_name, batch_size, mode, num_classes, data_dir, l_data, u_data, save_path, dropout=0.5,
                 max_frames=900):
    ...
    def select_top_data_vrm(self, pred_score, dists, select_num, vari_percent, vari_value):
        N_u, N_l = dists.shape
        stds = np.zeros(N_u)
        vari_idx = np.zeros(N_u, 'bool')
        for i in range(N_u):
            score = - dists[i]
        ...
        if vari_value == 0:  # 按照百分百划分值
            ...
            vari_num = math.floor(N_u * vari_percent)
            ...
            for i in index_vari:
                if stds[i] < vari_value:
                    vari_idx[i] = 1  # 这部分数据不可信

        select_credible = np.zeros(N_u, 'bool')
        select_uncredible = np.zeros(N_u, 'bool')
        index = np.argsort(-pred_score)
        for i in range(select_num):  # 总的选择数量不变
            if vari_idx[index[i]] == 1:  # 不可信
                select_uncredible[index[i]] = 1
            else:
                select_credible[index[i]] = 1
        num_of_credible = len(np.nonzero(select_credible)[0])
        num_of_uncredibel = len(np.nonzero(select_uncredible)[0])
        print("plan to select {} data, selected {} in actually. {} is credible and {} is uncredible".format(select_num,
                                                                                                            num_of_credible + num_of_uncredibel,
                                                                                                            num_of_credible,
                                                                                                            num_of_uncredibel))
    ...
    def estimate_label_atm3(self, u_data, l_data, one_shot):
        ...
        dists = []
        for idx, u_fea in enumerate(u_feas):
            dist_one = np.zeros((o_label.shape[0]))
            diffs = l_feas - u_fea
            dist = np.linalg.norm(diffs, axis=1)
            dists.append(dist)
    ...
    def select_top_data_nlvm_b5(self, pred_score, dists, new_nums_to_select, percent_vari):
        ...
        # 方案2, 求最近的P%样本的方差
        N_u, N_l = dists.shape
        vari_num = round(N_u * percent_vari)  # 取前面vari_num个
    ...
    def select_top_data_nlvm_b6(self, pred_score, dists, new_nums_to_select, percent_vari):
        ...
        for i in range(N_u):  # 对所有的值求方差
            ...
            stds[i] = score[topk_idxs].std()
        idxs = np.argsort(-stds)
        selection1[idxs[:vari_num]] = True
        index = np.argsort(-pred_score)  # 分数从大到小排序
        selection2[index[:new_nums_to_select]] = True
        selection = selection1 & selection2
        return selection, np.count_nonzero(selection)
    ...
    def generate_selected_data(self, sel_idx, pred_y):
