...
def main(args):
    ...
    # 输出该轮训练关键的提示信息
    print(
        "{} training begin with dataset:{},batch_size:{},epoch:{},step_size:{},max_frames:{},total_step:{},EF:{},q:{},P_e:{} P_q:{} stop_vari_step:{}".format(
            args.exp_name, args.dataset, args.batch_size, args.epoch, args.step_size, args.max_frames, total_step + 1,
            args.EF, args.q, args.P_e, args.P_q,args.stop_vari_step))
    ...
    if args.dataset == 'mars':
        expand_rate = [1.6, 1.5, 1.4, 1.3, 1.2, 1.1, 1.1]  # mars
    else:
        expand_rate = [1.1, 1.08, 1.06, 1.04, 1.02]  # duke
    if args.exp_name == 'vgssm_EF15':
        if args.dataset == 'mars':
            expand_rate = [1.2, 1.1, 1.08, 1.06, 1.04, 1.02]
        else:
            expand_rate = [1.3, 1.2, 1.1, 1.08, 1.06, 1.04, 1.02]
        args.stop_vari_step = len(expand_rate)
    while (not isout):
        print("{} training begin with dataset:{},batch_size:{},epoch:{},step:{}/{} saved to {}.".format(args.exp_name,
                                                                                                        ...
                                                                                                        save_path))
        print(
            "key parameters contain EF:{},q:{},P_e:{}, P_q:{}, stop_vari_step:{} Nums_been_selected:{}".format(args.EF,
                                                                                                               args.q,
                                                                                                               args.P_e,
                                                                                                               args.P_q,
                                                                                                               args.stop_vari_step,
        ...
        if step < args.stop_vari_step:
            real_percent_vari = expand_rate[step]
            ...
            real_percent_vari = 1
            new_expend_nums_to_select = new_nums_to_select
        ...
        data_file.write(
            "step:{} mAP:{:.2%} top1:{:.2%} top5:{:.2%} top10:{:.2%} top20:{:.2%} nums_selected:{} real_percent_vari:{} expend_nums_to_select:{} selected_percent:{:.2%} label_pre:{:.2%} select_pre:{:.2%}\n".format(
                int(step + 1), mAP, top1, top5, top10, top20, nums_to_select, real_percent_vari, expend_nums_to_select,
