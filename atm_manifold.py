...
def main(args):
    ...
    add_tagper_for_train = math.ceil(len(u_data)*args.add_to_tagper_percent)  # 这就是一个固定的数值了
    ...
    for step in range(total_step+1):
        ...
        reid.train(train_reid_data, step, tagper=0,epochs=args.epoch, step_size=args.step_size, init_lr=0.1) if step != resume_step else reid.resume(ckpt_file, step)
        # 开始评估
        ...
        tagper = reid  #对模型进行拷贝
        if len(l_data) == 0 : #表明这是第一次使用tagper
            if args.resume:
                ...
                selected_idx = reid.select_top_data(pred_score,add_tagper_for_train)  #这里是按照add_tagper_for_train的比例来进行选择,默认全选
        ...
        if len(u_data) <= mv_num:
            mv_num = len(u_data)
        selected_idx = tagper.select_top_data(pred_score,mv_num)  # 从所有 u_data 里面选
