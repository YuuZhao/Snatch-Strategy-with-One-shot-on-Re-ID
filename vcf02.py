...
def main(args):
    ...
    # 输出该轮训练关键的提示信息
    print(
        "{} training begin with dataset:{},batch_size:{},epoch:{},step_size:{},max_frames:{},total_step:{},EF:{},q:{},P_e:{} P_q:{}".format(
            args.exp_name, args.dataset, args.batch_size, args.epoch, args.step_size, args.max_frames, total_step + 1,
            args.EF, args.q, args.P_e, args.P_q))
    ...
    while (not isout):
        print("{} training begin with dataset:{},batch_size:{},epoch:{},step:{}/{} saved to {}.".format(args.exp_name,
                                                                                                        ...
                                                                                                        save_path))
        print(
            "key parameters contain EF:{},q:{},P_e:{}, P_q:{},  Nums_been_selected:{}".format(args.EF,
                                                                                                               args.q,
                                                                                                               args.P_e,
                                                                                                               args.P_q,
                                                                                                               nums_to_select))
        ...
        # nums_to_select 的设定
        new_nums_to_select = min(math.ceil(len(u_data) * math.pow((step + 1), args.q) * args.EF / 100),
                                 len(u_data))  # EUG 基础指数渐进策略



        new_expend_nums_to_select = min(len(u_data), math.ceil(new_nums_to_select / args.P_e))
        ...
        # 输出该epoch的信息
        data_file.write(
            "step:{} mAP:{:.2%} top1:{:.2%} top5:{:.2%} top10:{:.2%} top20:{:.2%} nums_selected:{} new_query_nums:{} expend_nums_to_select:{} selected_percent:{:.2%} label_pre:{:.2%} select_pre:{:.2%}\n".format(
                int(step + 1), mAP, top1, top5, top10, top20, nums_to_select, new_query_nums, expend_nums_to_select,
                nums_to_select / len(u_data), label_pre, select_pre))
        print(
            "step:{} mAP:{:.2%} top1:{:.2%} top5:{:.2%} top10:{:.2%} top20:{:.2%} nums_selected:{} new_query_nums:{}  expend_nums_to_select:{}  selected_percent:{:.2%} label_pre:{:.2%} select_pre:{:.2%}\n".format(
                int(step + 1), mAP, top1, top5, top10, top20, nums_to_select, new_query_nums, expend_nums_to_select,
                nums_to_select / len(u_data), label_pre, select_pre))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default="vgssm")  # 方差逐渐停止采样法.
    ...
    parser.add_argument('--stop_vari_step', type=int, default=11)  # 默认不停止
    parser.add_argument('--P_e', type=float, default=0.9)
    parser.add_argument('--P_q', type=float, default=0.9)
    ...
    '''
    python3.6 vcf02.py --exp_order 5 --P_e 0.8 --P_q 0.8  
    python3.6 vcf02.py --exp_order 6 --P_e 0.9 --P_q 0.8  
