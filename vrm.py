...
def main(args):
    ...
    while (not isout):
        ...
        pred_y, pred_score, label_pre, dists = eug.estimate_label_atm3(u_data, credible_data, l_data)
        ...
        selected_credible, selected_uncredible, vari_value, num_of_credible, num_of_uncredibel = eug.select_top_data_vrm(
            pred_score, dists,
            new_nums_to_select,
            args.vari_percent, vari_value)
        credible_data, select_pre_cre = eug.generate_selected_data(selected_credible, pred_y)
        uncredible_data, select_pre_uncre = eug.generate_selected_data(selected_uncredible, pred_y)
        new_select_data = credible_data + uncredible_data
        select_pre = (select_pre_cre * num_of_credible + select_pre_uncre * num_of_uncredibel) / (
                num_of_credible + num_of_uncredibel)
        new_train_data = new_select_data + l_data
